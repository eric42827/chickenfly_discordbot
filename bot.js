...
client.on('message', async message => {
    ...
    if (message.content.substring(0, 1) === "!" || message.content.substring(0, 1) === "！") {
        ...
        if (message.author.id in userlist && message.channel.id in chlist) {
            ...
            if (command === 'fill' || command === '填表' || command === '傷害' || command === 'f') {
                queue.push(async () => {
                    try {
                        memberid = message.author.id;
                        demage = parseInt(args[0]);
                        if (isNaN(demage) || demage > 99999999) {
            ...
            else if (command === 'fillfor' || command === '代填' || command === '幫填' || command === 'ff') {
                queue.push(async () => {
                    try {
            ...
            else if (command === '報數' || command === '報'  || command === '1') {
                queue.push(async () => {
                    try {
                        var tables = await gapi.getCollectingtable(chlist[message.channel.id]);
            ...
            else if (command === '進場' || command === '進' || command === '2') {
                queue.push(async () => {
                    try {
                        var tables = await gapi.getCollectingtable(chlist[message.channel.id]);
                        memberName = userlist[message.author.id][0];
